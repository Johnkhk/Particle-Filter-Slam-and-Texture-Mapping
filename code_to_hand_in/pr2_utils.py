...
def mapCorrelation(im, x_im, y_im, vp, xs, ys):
  ...
  # count = np.count_nonzero(im == 1)
  # print("count1", count)
  for jy in range(0,nys):
    y1 = vp[1,:] + ys[jy] # 1 x 1076
    # print("ys",ys[jy])
    iy = np.int16(np.round((y1-ymin)/yresolution))
    # iy = np.int16(np.round((y1)))
    for jx in range(0,nxs):
      x1 = vp[0,:] + xs[jx] # 1 x 1076
      ix = np.int16(np.round((x1-xmin)/xresolution))
      # ix = np.int16(np.round((x1)/xresolution))
      valid = np.logical_and( np.logical_and((iy >=0), (iy < ny)), \
			                        np.logical_and((ix >=0), (ix < nx)))
      # print("iy, ix",iy,ix)                              
      # print("valid",valid)
      # print("valid",valid.shape)
      # print("sumsum", np.sum(im[ix[valid],iy[valid]]))
      cpr[jx,jy] = np.sum(im[ix[valid],iy[valid]])
  return cpr

def bresenham2DVectorizedForFullScan(sx, sy, ex, ey):
  '''
...
def test_mapCorrelation():
  ...
  # count = np.count_nonzero(MAP['map'] == 1)
  # print("count222", count)
      
  x_im = np.arange(MAP['xmin'],MAP['xmax']+MAP['res'],MAP['res']) #x-positions of each pixel of the map
  y_im = np.arange(MAP['ymin'],MAP['ymax']+MAP['res'],MAP['res']) #y-positions of each pixel of the map

  x_range = np.arange(-0.4,0.4+0.1,0.1) # xcoord is 0 here, *resolution
