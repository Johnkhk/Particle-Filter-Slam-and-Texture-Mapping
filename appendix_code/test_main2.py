    ...
    def predict(self, plot=False): #noise params
        for p in range(1):
            ...
            for i in range(1, len(self.lidar_data[:,0])):
                # if i==20000:
                    # break
                self.differentialDrivePredict(i)
                if i%100!=0:
                    continue
                # self.runBresenham()
                # self.runMapCorrel()
        ...
        if plot:
            ...
            plt.show(block=True)
    def differentialDrivePredict(self, i)  :
        ...
        tau = time_diff1
        # self.theta += tau* ( (self.fog_data[i-1][-1] + noise.gaussNoise(0,self.angNoiseCovar,1)[0]) / tau)
        self.theta += ( (self.fog_data[i-1][-1] + noise.gaussNoise(0,self.angNoiseCovar,1)[0]))

        self.xcar += (tau*lin_vel* (math.cos(self.theta)) )
        self.ycar += (tau*lin_vel* (math.sin(self.theta)) )
        self.pred.append((self.xcar,self.ycar, self.theta))
    def runBresenham(self):
        self.bresStack=np.empty((2,0))
        for i in range(len(self.lidarPointsInWorld)):
            # print("YOOOOO", self.xcar, self.lidarPointsInWorld[i][0], self.ycar, self.lidarPointsInWorld[i][1])
