...
slam = slam(100, (-300, 1400), (-1100, 800), 1)
...
for i in range(1, len(data.lidar_data[:,0])):
    ...
    for j in range(slam.numParticles):
        ...
        lidarInWorldParticles[j] = Rotate2D(u[j,2], lidarInWorldParticles[j])
    ...
    for p in range(slam.numParticles):
        ...
        # lidarInW1 = np.array([uxy[p,0],uxy[p,1]]) + lidarInV
        lidarInW1 = Rotate2D(u[p,2], lidarInV)

        maxc, idx, particle = runMapCorrel(p, uxy[p,0], uxy[p,1], x_im, y_im, lidarInW1, slam.mapLogOdds.MAP['map'], slam.bias, False)
        Cnorm += (slam.weights[p]*maxc)
        # print("weights",slam.weights)
        # print(maxc)
        # print("browut",slam.weights[p]*maxc, slam.weights[p], maxc)
    ...
    # lidar_tmp_inW = Rotate2D(particlepose[2], lidar_tmp_inW)

    # lidar_tmp_inW = np.array([particlepose[0],particlepose[1]]) 
    # lidar_tmp_inW = Rotate2D(particlepose[2], lidar_tmp_inW) + lidarInV
    # lidar_tmp_inW = lidar_tmp_inW[slam.mapLogOdds.in_map(lidar_tmp_inW),:]
